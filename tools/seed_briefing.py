# -*- coding: utf-8 -*-
"""
把人工精读内容写成博客式长文（含论文主图与图注），写入当日 JSON。

条目字段：
    section / klass  —— 决定分区与「干员职业」
    essay            —— 有序内容块：lead 引入 / h 分节标题 / p 段落 / fig 配图 / quote 摘录 / close 总结
运行：
...
FIGMAN = os.path.join(ROOT, "assets", "figures", "manifest.json")
...
def fig(aid, cap, cap_en=""):
    """按 arXiv 编号引用 assets/figures 下已下载的主图。"""
    if not os.path.exists(FIGMAN):
        return None
    man = json.load(open(FIGMAN))
    m = man.get(aid)
    if not m:
        print(f"[warn] {aid} 无主图，已跳过配图块")
        return None
    return {"t": "fig", "src": m["local"], "cap": cap, "cap_en": cap_en or m.get("caption", "")}
...
add(
    ...
    essay=[
        ...
        fig("2609.15361",
            "MARC 的通信架构：消息编码器挂在任意值函数分解方法之上（图中 mixer 模块），全参数共享；粉色菱形为消息正则化单元。",
            "Fig. 2: Architecture of the proposed communication method for MARL."),
...
add(
    ...
    essay=[
        ...
        fig("2609.14959",
            "战略性在线作业调度：各玩家随时间收到作业，自行选择投给哪几台机器；机器是共享资源，本地处理条件随机演化。",
            "Figure 1: Strategic online job scheduling on stochastic machines."),
...
add(
    ...
    essay=[
        ...
        fig("2609.17601",
            "以局部收益之和作为社会福利时的学习轨迹（原图 (a) 面板）：agents 在无中心协调下自发选出社会合意的均衡。",
            "(a) Sum of Local Payoffs as Social Welfare"),
...
add(
    ...
    essay=[
        ...
        fig("2609.18639",
            "on-policy 训练动态：(a) 回合奖励、(b) 网络日均服务量、(c) 日均服务差距；曲线为百回合滑动平均。",
            "Figure 2: On-policy training dynamics: (a) episodic reward, (b) mean daily network service, (c) mean daily service gap."),
        {"t": "h", "v": "一个更诚实的实验设定"},
...
add(
    ...
    essay=[
        ...
        fig("2609.19820",
            "可控性（E2）：实际选中的坐标 c_sel 对目标坐标 c_target，跨五个博弈、固定参考与细化参考两种设置（多种子均值 ± 标准差）；虚线为恒等线，点线为最大熵成员。",
            "Figure 1: Controllability (E2). Achieved coordinate vs requested for fixed and refined references."),
...
add(
    ...
    essay=[
        ...
        fig("2609.19399",
            "在每轮 5% 收益预算下、十二轮 PSRO 迭代中的可剥削性曲线：RWPS 在同预算下低于 minimum-regret-first、information-gain 与 progressive sampling。",
            "Figure 1: Exploitability against the full game over twelve PSRO iterations at a 5% per-build payoff budget."),
        {"t": "h", "v": "能提前预判问题贵不贵"},
...
add(
    ...
    essay=[
        ...
        fig("2609.16270",
            "Qwen 上实验 1 的机制拆解：每根柱子把「预期切换率的下降」拆成轮间 P(A) 漂移减少（蓝）与行动不确定性降低（橙）两部分，菱形为两者之和。",
            "Figure 2: Experiment 1 decomposition for Qwen — reduced between-round drift (blue) and reduced action uncertainty (orange)."),
        {"t": "h", "v": "不止相关，还给因果证据"},
...
add(
    ...
    essay=[
        ...
        fig("2609.18591",
            "多智能体系统中的关系复杂性：当每个人都在持续影响其他人时，单次决策就不再是简单逻辑，而变成一层层相互猜测的递归循环。",
            "Figure 1: Relational Complexity in Multi-Agent Systems."),
...
add(
    ...
    essay=[
        ...
        fig("2609.19124",
            "Flag Game 示意：隐藏旗帜只能通过私有证据观测，有界 agents 交换报告，系统追踪各 agent 的计数与信念演化。",
            "Figure 1: The Flag Game — a hidden flag observed through private evidence; bounded agents exchange reports."),
        {"t": "h", "v": "小群体坍缩，大群体极化"},
...
add(
    ...
    essay=[
        ...
        fig("2609.17320",
            "Emergence World 城镇俯视图：市政厅、中央银行、凯旋门、公共图书馆、警察局与 Agent TechHub 等关键制度性地标；住宅与其他地标从略。",
            "Figure 2: Aerial view of the Emergence World town, with key institutional landmarks shown."),
...
add(
    ...
    essay=[
        ...
        fig("2609.15803",
            "阈值规则的近似保证：每个容忍度组合下，颜色给出同时满足两项保证的响应 (a) 或提示 (b) 的最大比例（在整个基准上统一选取 k）。",
            "Figure 7: Approximate threshold-rule guarantees — greatest fraction of responses (a) or prompts (b) satisfying both guarantees."),
...
add(
    ...
    essay=[
        ...
        fig("2609.17856",
            "攻击与防御的定性可视化（受害者融合场景视角）：攻击者 1 在 matched-objective harness 下扰动其发送的特征；计数为 IoU 0.5 下恢复出的真实物体数。",
            "Figure 2: Qualitative visualization of attacks and the defenses, seen on the fused scene by victim."),
        {"t": "h", "v": "从离线优化玩具，到实时威胁"},
...
add(
    ...
    essay=[
        ...
        fig("2609.19789",
            "通用多智能体交易系统（GMATS）架构：投毒内容从分析师层的证据流进入，经协调层汇总后影响最终执行。",
            "Figure 1: Architecture of the Generic Multi-Agent Trading System (GMATS)."),
        {"t": "h", "v": "量化「传染」，以及拓扑即防御"},
...
data["note"] = (
    ...
    "经四项筛选标准精读摘要后精选 16 篇；解读为博客式图文长文，配图取自论文原图。"
    "下方雷达为脚本自动抓取的补充条目。"
)
data["curated"] = True
...
nfig = sum(1 for h in H for b in h["essay"] if b and b["t"] == "fig")
print(f"写入 {len(H)} 篇解读（含 {nfig} 张配图）+ {len(TRENDS)} 条趋势 -> {PATH}")
